...
export const Home: React.FC = () => {
    ...
    const taskCreated = (task: Partial<Task>): void => {
        let lastId = project!.tasks!.reduce((ant, t) => { return t.id > ant.id ? t : ant }).id;
        let maxOrder = project!.tasks!.reduce((ant, t) => { return t.order > ant.order ? t : ant }).order;
        let newTask = task;
        newTask.id = lastId + 1;
        newTask.done = false;
        newTask.order = maxOrder + 1;
        let tempTasks = project!.tasks || [];
        tempTasks.push(newTask as Task);
        setProject((previous) => (({ ...previous!, tasks: tempTasks })));
        setTasks(tempTasks);
    }

    const deleteTask = (task: Task): void => {
        let tasksTemp = project!.tasks!.filter(t => t.name !== task.name);
        setProject((previous) => ({ ...previous!, tasks: tasksTemp }));
        setTasks(tasksTemp);
    }
        ...
        <>
            <div className="homepage">
                ...
                <div className="content">
                    <TaskFilter onSendData={receberFiltros} updateProject={project!} onlyActives={setOnlyActives} activeTag={setActiveTag} />
                    <div className="div-projeto">
                        <div className="box-titulo">
                            <h3 className="titulo-tarefas">Lista da Tarefas <button onClick={() => setShowNewTask(true)} className="btn-add-tarefa" title="Nova tarefa">✚</button></h3>
                            <div>
                        ...
                        {showNewTask ? <div className="newtask-box">
                            <NewTask opened={setShowNewTask} taskCreated={taskCreated}></NewTask>
                        </div>
                            :
